...
$start = DateTime::createFromFormat('Y-m-d', $startParam);
$end   = DateTime::createFromFormat('Y-m-d', $endParam);
...
if ($startFormatted && $endFormatted) {
  $allcount = $link->prepare("
    SELECT *
    FROM wp_gf_entry
    WHERE status = ?
    AND date_created BETWEEN ? AND ?
  ");
  $allcount->bind_param("sss", $status, $startFormatted, $endFormatted);
} else {
  $allcount = $link->prepare("
    SELECT *
    FROM wp_gf_entry
...
foreach($genrearray as $data){
  if ($startFormatted && $endFormatted) {
    $genrecount = $link->prepare("
      SELECT m.*
      FROM wp_gf_entry_meta m
      JOIN wp_gf_entry e ON e.id = m.entry_id
      WHERE m.meta_value = ?
      AND e.date_created BETWEEN ? AND ?
      AND e.status = 'active'
    ");
    $genrecount->bind_param("sss", $data, $startFormatted, $endFormatted);
  } else {
    $genrecount = $link->prepare("
      SELECT m.*
      FROM wp_gf_entry_meta m
...
foreach($consolearray as $data){
  if ($startFormatted && $endFormatted) {
    $consolecount = $link->prepare("
      ...
      AND e.status = 'active'
    ");
    $consolecount->bind_param("sss", $data, $startFormatted, $endFormatted);
  } else {
    $consolecount = $link->prepare("
      SELECT m.*
      FROM wp_gf_entry_meta m
      ...
      AND e.status = 'active'
    ");
    $consolecount->bind_param("s", $data);
  }
...
foreach($gamearray as $data){
  if ($startFormatted && $endFormatted) {
    $gamecount = $link->prepare("
      ...
      AND e.status = 'active'
    ");
    $gamecount->bind_param("sss", $data, $startFormatted, $endFormatted);
  } else {
    $gamecount = $link->prepare("
      SELECT m.*
      FROM wp_gf_entry_meta m
    ...
    <div class="col-auto">
      <label class="form-label">From
        <input type="date" name="start" class="form-control"
               value="<?=htmlspecialchars($startParam)?>" required>
      </label>
    </div>
    <div class="col-auto">
      <label class="form-label">To
        <input type="date" name="end" class="form-control"
               value="<?=htmlspecialchars($endParam)?>" required>
      </label>
    </div>
    <div class="col-auto">
      <button type="submit" class="btn btn-primary">Search</button>
      <?php if ($startParam && $endParam): ?>
