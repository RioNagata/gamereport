...
$allcount = $link->prepare($sql);

$like = $date . "%"; // e.g., "2025-01%"
$allcount->bind_param("s", $like);

$allcount->execute();
$allresult = $allcount->get_result();
$allresultcount = $allresult->num_rows;
...
foreach ($genrearray as $data) {
    // join meta with entry to filter by date_created
    $sql = "SELECT m.* 
    ...
    $genrecount = $link->prepare($sql);

    // bind both params safely
    $like = $date . "%"; // e.g., "2025-01%"
    $genrecount->bind_param("ss", $data, $like);

    $genrecount->execute();
    $genreresult = $genrecount->get_result();
    $genreresultcount = $genreresult->num_rows;
...
foreach ($consolearray as $data) {
    ...
    $consolecount = $link->prepare($sql);

    $like = $date . "%"; // e.g. "2025-01%"
    $consolecount->bind_param("ss", $data, $like);

    $consolecount->execute();
    $consoleresult = $consolecount->get_result();
    $consoleresultcount = $consoleresult->num_rows;
...
foreach ($gamearray as $data) {
    ...
    $gamecount = $link->prepare($sql);

    $like = $date . "%"; // e.g. "2025-01%"
    $gamecount->bind_param("ss", $data, $like);

    $gamecount->execute();
    $gameresult = $gamecount->get_result();
    $gameresultcount = $gameresult->num_rows;
